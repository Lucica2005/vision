...
def generate_train_json():
    ...
    # 获取前20个文件夹
    image_folders = sorted([d for d in os.listdir(images_path) if os.path.isdir(os.path.join(images_path, d))])[:20]
    ...
    coco_data = {
        "info": {
            "description": "TiaoZhanCup Dataset",
            "url": "",
            "version": "1.0",
            "year": 2024,
    ...
    # 保存JSON文件
    output_path = os.path.join(base_path, "annotations/train.json")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(coco_data, f, indent=2, ensure_ascii=False)
    
    print(f"\nGenerated train.json with:")
